...
const teamMembers = [
  {
    name: "AKILA DHAMBURE",
    role: "WEB DEVELOPER",
    image: "team_akila", // Now references the key in assets.js
    social: {
  ...
  },
  {
    ...
    role: "WEB DEVELOPER",
    image: "team_pamith", // Now references the key in assets.js
    social: {
  ...
  },
  {
    ...
    role: "WEB DEVELOPER",
    image: "team_vinindu", // Now references the key in assets.js
    social: {
...
const OurTeam = ({ isDarkMode }) => {
  return (
    <section id="team" className={`py-20 px-4 sm:px-6 lg:px-8 text-center ${isDarkMode ? 'bg-gray-900' : 'bg-gray-100'} transition-colors duration-300`}> {/* Adjusted section background for consistency with the UI */}
      {/* "OUR TEAM" button */}
      <motion.button
      ...
      {/* Grid for team member profiles (no individual cards) */}
      <div className="grid max-w-6xl grid-cols-1 mx-auto md:grid-cols-2 lg:grid-cols-3 gap-y-12"> {/* Adjusted gap-y for vertical spacing */}
        {teamMembers.map((member, index) => (
          <motion.div
            key={index}
            initial={{ opacity: 0, y: 50 }}
            whileInView={{ opacity: 1, y: 0 }}
            whileHover={{ y: -5 }} // Subtle upward movement on hover for the whole profile block
            transition={{ duration: 0.6, delay: index * 0.1 }}
            viewport={{ once: true }}
            className="flex flex-col items-center justify-center cursor-pointer" // Centering content within each grid item
          >
            {/* Team member image - now only the circular image without extra borders */}
            <div className="flex items-center justify-center mx-auto mb-6 overflow-hidden rounded-full w-36 h-36"> {/* Slightly increased size */}
              <Image
                src={assets[member.image]}
                alt={member.name}
                width={144} // Corresponding to w-36 (144px)
                height={144} // Corresponding to h-36 (144px)
                objectFit="cover"
                className="rounded-full"
                onError={(e) => {
                  e.target.onerror = null;
                  e.target.src = "https://placehold.co/144x144/CCCCCC/666666?text=Image+Error";
                }}
              />
            </div>

            {/* Team member name */}
            <h3 className={`font-sans text-xl sm:text-2xl font-bold mb-1 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}> {/* Adjusted color to be white in dark mode, matching UI */}
              {member.name}
            </h3>
            {/* Team member role */}
            <p className={`text-base sm:text-lg font-semibold mb-6 ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
              {member.role}
            </p>

            {/* Social Media Links */}
            <div className="flex justify-center space-x-4">
              <a href={member.social.facebook} target="_blank" rel="noopener noreferrer" className={`${isDarkMode ? 'text-gray-400 hover:text-blue-400' : 'text-gray-600 hover:text-blue-700'} transition-colors duration-200`}>
                {/* Facebook Icon SVG */}
              ...
              <a href={member.social.linkedin} target="_blank" rel="noopener noreferrer" className={`${isDarkMode ? 'text-gray-400 hover:text-blue-400' : 'text-gray-600 hover:text-blue-700'} transition-colors duration-200`}>
                {/* LinkedIn Icon SVG */}
                <svg className="w-7 h-7" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                  <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"/>
                </svg>
              </a>
              <a href={member.social.instagram} target="_blank" rel="noopener noreferrer" className={`${isDarkMode ? 'text-gray-400 hover:text-blue-400' : 'text-gray-600 hover:text-blue-700'} transition-colors duration-200`}>
                {/* Instagram Icon SVG */}
                <svg className="w-7 h-7" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
              ...
              <a href={member.social.whatsapp} target="_blank" rel="noopener noreferrer" className={`${isDarkMode ? 'text-gray-400 hover:text-blue-400' : 'text-gray-600 hover:text-blue-700'} transition-colors duration-200`}>
                {/* WhatsApp Icon SVG */}
                <svg className="w-7 h-7" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
